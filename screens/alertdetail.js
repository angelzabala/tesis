...
import { Dimensions, StyleSheet, ScrollView, View, Image, Text, TextInput, TouchableWithoutFeedback, Linking } from 'react-native';
...
export default class alertScreenComponent extends React.Component {
        ...
        if (this.state.alerta.falsa == 'true') {
            return (
                <View style={styles.fakeAlermContainer}>
                    ...
                    <Text style={styles.smallRedText}>Hora de reporte: {formattedHour}</Text>
                </View>
            )
        } else {
            ...
            if (this.state.alerta.nivel == 1) {
                return (
                    <View style={{ display: 'flex', flexDirection: 'row' }}>
                        <Text style={{ alignSelf: 'center', marginRight: screenWidth * 0.15, fontWeight: 'bold', fontSize: 30 }}>{formattedHour}</Text>
    ...
    mapComponent = () => {
        if (this.state.nivel == 3) {
            return (
                <TouchableWithoutFeedback onLongPress={() => { Linking.openURL('https://google.com/maps/?q=' + this.state.alerta.latitud + ',' + this.state.alerta.longitud) }}>
                    <MapView
                        style={{ flex: 1, width: screenWidth * 0.8, height: screenWidth * 0.6, marginBottom: 20, marginTop: 20 }}
                        initialRegion={{
    ...
    getAlertData = () => {
        ...
        fetch(ipv4 + '/alerta/' + this.state.navigation.state.params.pk_alerta, options).then(response => {
    ...
    sendReport = (fk_alerta) =>{
        this.state.navigation.navigate('addReportScreen', {alerta: this.state.alerta});
    }
    ...
    componentDidMount() {
        ...
        this.subs = [
            this.props.navigation.addListener("didFocus", () => {
                this.setState({ isFocused: true }); this.getAlertData(); 
            }),
        ...
        return (
            <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }}>
                <View style={styles.container}>
                    <Text style={{ fontWeight: 'bold', fontSize: 30, alignSelf: 'flex-start', marginTop: 30, marginLeft: screenWidth * 0.1, marginBottom: 20 }}>
                        Del grupo: {this.state.alerta.nombre}
                    </Text>
                    <Text style={{ marginBottom: 20 }}>reportado por el usuario: {this.state.alerta.fk_usuario}</Text>
                    {this.circleLevelComponent(this.state.alerta.hora)}
                    <View>
                        <Text style={styles.inputLabel}>LUGAR</Text>
                        <TextInput editable={false} value={this.state.alerta.lugar} autoCompleteType="email" onChangeText={(value) => { }} style={styles.regularInputText} />
                    </View>
                    {this.mapComponent()}
                    <View>
                        <Text style={styles.inputLabel}>Descripción</Text>
                        <TextInput editable={false} value={this.state.alerta.descripcion} autoCompleteType="email" onChangeText={(value) => { }}
                            style={styles.fakeTextArea} multiline={true} numberOfLines={4} />
                    </View>
                    <View style={styles.buttonContainer}>
                        <TouchableWithoutFeedback onPress={() => { this.sendReport(this.state.navigation.state.params.pk_alerta) }}>
                            <Text style={{ color: "#fff", "backgroundColor": "#012133", textAlign: "center", "borderRadius": 25, padding: 10 }}>Agregar reporte a esta alerta</Text>
                        </TouchableWithoutFeedback>
                    </View>
                </View>
            </ScrollView>
