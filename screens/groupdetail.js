...
import { Dimensions, StyleSheet, FlatList, View, Image, Text, TouchableWithoutFeedback, SafeAreaView, AsyncStorage } from 'react-native';
...
export default class groupDetailComponent extends React.Component {
    ...
    groupUsers = async () => {
        ...
        fetch(ipv4 + '/u-grupos/' + data.phone + '/' + data.fk_grupo + '/' + data.isAdmin, options).then(response => {
    ...
    reportHandler =() =>{
        this.state.navigation.navigate('groupReports', {fk_grupo: this.props.navigation.state.params.groupId}) ;
    }
    statisticsHandler = () =>{
        this.state.navigation.navigate('statisticsScreen', {fk_grupo: this.props.navigation.state.params.groupId}) ;
    }
    ...
    render() {
        if (this.state.users == '' || this.state.users.length <= 1/*cambiar por 1*/) {
            return ( 
                <SafeAreaView style={styles.container}>
                    <TouchableWithoutFeedback >
                        <View>
                            <Text style={styles.groupName}>{this.props.navigation.state.params.groupName}</Text>
                            <Text style={styles.noUsersText}>el grupo no tiene miembros agregados, para agregarlos, toque el ícono de arriba a la derecha</Text>
                        </View>
                    </TouchableWithoutFeedback>
                </SafeAreaView>
            )
            ...
            return (
                <SafeAreaView style={styles.container}>
                    ...
                    <View style={styles.alertContainer}>
                        <TouchableWithoutFeedback style={styles.alertBox} onPress={() => {this.sendAlert(1)}}>
                        ...
                        <TouchableWithoutFeedback style={styles.alertBox} onPress={() => {this.sendAlert(2)}}>
                        ...
                        <TouchableWithoutFeedback style={styles.alertBox} onPress={() => {this.sendAlert(3)}}>
                    ...
                    <View style={styles.userSectionContainer}>
                        ...
                        <FlatList data={this.state.users}
                            keyExtractor={item => item.pk_usuariogrupo.toString()} renderItem={({ item }) => {
                                if (item.fk_usuario != this.state.phone) {
                                    return (
                                        <TouchableWithoutFeedback onPress={() => { this.userPressHandler(item) }}>
                                            <View style={{ marginBottom: 20 }}>
                                                <View style={styles.nameContainer}>
                                                    <Text style={styles.userName}>{item.nombre} - {item.fk_usuario}</Text>
                                                </View>
                    ...
                    <View style={styles.fakeFixedFooter}>
                        <TouchableWithoutFeedback onPress={() => { this.reportHandler()}}>
                            <View style={styles.footerObjectContainer}>
                                <Text style={styles.footerLabel}>Reportes</Text>
                                <Image style={styles.footerIcon} source={require('../assets/iconosFigma/summary.png')} />
                            </View>
                        </TouchableWithoutFeedback>
                        <TouchableWithoutFeedback onPress={() => { this.statisticsHandler() }}>
                            <View style={styles.footerObjectContainer}>
                                <Text style={styles.footerLabel}>Estadísticas</Text>
                                <Image style={styles.footerIcon} source={require('../assets/iconosFigma/statistics.png')} />
                            </View>
                        </TouchableWithoutFeedback>
                    </View>
                </SafeAreaView>
