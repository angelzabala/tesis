...
export default class alertScreenComponent extends React.Component {
        ...
        if(this.state.falsa == 'true'){
            return(
                <View style={styles.fakeAlermContainer}>
                    ...
                    <Text style={styles.smallRedText}>Reportada falsa a las: {format(new Date(), 'H:mma')}</Text>
                </View>
            )
        }else{
    ...
    mapComponent = () => {
        if (this.state.nivel == 3) {
            return (
                <TouchableWithoutFeedback onLongPress={() => { Linking.openURL('https://google.com/maps/?q=' + this.state.navigation.state.params.latitud + ',' + this.state.navigation.state.params.longitud) }}>
                    <MapView
    ...
    componentDidMount() {
        ...
        socket.on('updateDatabase', (data)=> {this.updateAlertManually();})
    }
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }}>
                <View style={styles.container}>
                    <Modal isVisible={this.state.estado == 'false'} style={styles.modal}>
                        <Text style={styles.modalTitle}>Evento finalizado, presione el boton para salir</Text>
                        <View style={styles.buttonContainer}>
                        <TouchableWithoutFeedback onPress={() => { this.eventExitHandler() }}>
                            <Text style={{ color: "#fff", "backgroundColor": "#012133", textAlign: "center", "borderRadius": 25, padding: 10 }}>Salir de la alerta</Text>
                        </TouchableWithoutFeedback>
                    </View>
                    </Modal>
                    ...
                    <View>
                        ...
                        <TextInput editable={false} value={this.state.lugar} autoCompleteType="email" onChangeText={(value) => { }} style={styles.regularInputText} />
                    </View>
                    {this.mapComponent()}
                    <View>
                        <Text style={styles.inputLabel}>Descripción</Text>
                        <TextInput editable={false} value={this.state.descripcion} autoCompleteType="email" onChangeText={(value) => { }}
