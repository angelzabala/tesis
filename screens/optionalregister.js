...
import { StyleSheet, TextInput, View, Text, TouchableWithoutFeedback, ScrollView, AsyncStorage, Dimensions } from 'react-native';
...
export default class OptionalInfoComponent extends React.Component{
	...
	registrationHandler = () => {
		...
		fetch(ipv4 + '/usuarios', options).then((response) => {
			if (response.status == 200) {
				AsyncStorage.setItem('phone', this.state.navigation.state.params.userData.phone).then(this.state.navigation.navigate('Home'));
			} else {
			...
			<ScrollView style={{ backgroundColor: '#04a5ff' }}>
				<View style={styles.container}>
					...
					<View style={styles.phoneContainer}>
						<View style={styles.pickerContainer}>
							<Picker
								...
								style={styles.picker}
								onValueChange={(itemValue, itemIndex) => this.setState({selectedPhone1:itemValue})}>
								<Picker.Item label="" value=""/>
						...
						<View>
							<TextInput placeholder='1234567' onChangeText={(value) => this.setState({phoneNumber1:value})}
					...
					<View style={styles.phoneContainer}>
						<View style={styles.pickerContainer}>
							<Picker
								...
								style={styles.picker}
								onValueChange={(itemValue, itemIndex) => this.setState({selectedPhone2: itemValue})}>
								<Picker.Item label="" value=""/>
								<Picker.Item label="0412" value="0412" />
								<Picker.Item label="0414" value="0414" />
								<Picker.Item label="0416" value="0416" />
					...
					<View>
						...
						<TextInput onChangeText={(value) => this.setState({allergies:value})} style={styles.regularInputText} />
					</View>

					<View>
						<Text style={styles.inputLabel}>ENFERMEDADES CRÓNICAS</Text>
						<TextInput onChangeText={(value) => this.setState({cronic:value})} style={styles.regularInputText} />
					</View>

					<View>
						<Text style={styles.inputLabel}>DISCAPACIDAD</Text>
						<TextInput onChangeText={(value) => this.setState({discapacity:value})} style={styles.regularInputText} />
					</View>

					<View style={styles.buttonContainer}>
						<TouchableWithoutFeedback style={styles.buttonContainer} onPress={this.registrationHandler}>
							<Text style={{ width: screenWidth*0.6,color: "#fff", "backgroundColor": "#012133", textAlign: "center", "borderRadius": 25, padding: 10 }}>Registrarse</Text>
