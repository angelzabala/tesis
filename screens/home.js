...
import {
    StyleSheet, Image, View, Text, FlatList, TouchableWithoutFeedback, AsyncStorage, Dimensions,
    SafeAreaView
} from 'react-native';
...
export default class HomeComponent extends React.Component {
    constructor(props) {
        ...
        /*this.socket = io(ipv4, {jsonp: false});*/
        this.state = {
            userPhoneNumber: '',
            groupsNumber: 0,
            groups: '',
    ...
    userRecentAlerts = () => {
        ...
        fetch(ipv4 + '/alertas/' + this.state.userPhoneNumber, options).then(response => {
    ...
    userGroups = () => {
        ...
        fetch(ipv4 + '/u-grupos/' + this.state.userPhoneNumber, options).then(response => {
            if (response.status == 200) {
                response.json().then(jsonObj => {
    ...
    falsaLabelComponent = (isFalse, date) => {
        ...
        if (isFalse) {
            return (<Text style={styles.fakeLabel}>{formattedDate} - {formattedHour} (falsa)</Text>)
        } else {
            return (<Text style={styles.fakeLabel}>{formattedDate} - {formattedHour}</Text>)
        }
    }
    ...
    alertPressHandler = (user, level, groups) => {
        navigator.geolocation.getCurrentPosition(
            ({ coords: { latitude, longitude } }) => { 
                ...
                let alertInformation = { user, level, groups, latitud: this.state.latitude, longitud: this.state.longitude };
        ...
        if (this.state.alerts.length == 0) {
            return (
                <View style={styles.groupSectionContainer}>
                    ...
                    <Text style={styles.noAlertsText}>no existen alertas previas por el momento</Text>
                </View>
            )
        } else {
            return (
                ...
                <View style={styles.groupSectionContainer}>
                    <Text style={styles.groupsLabel}>Alertas Recientes</Text>
                    <FlatList data={this.state.alerts.slice(0,3)}
                        keyExtractor={item => item.pk_alerta.toString()} renderItem={({ item }) => (
                            <TouchableWithoutFeedback onPress={() => {this.state.navigation.navigate('alertDetail', {pk_alerta: item.pk_alerta}) }}>
                                <View style={styles.recentAlertContainer}>
                                    <View style={styles.recentAlertInfo}>
                                        {this.circleLevelComponent(item.nivel)}
                                        <Text style={styles.recentAlertText}>{item.nombre} - {item.fk_usuario}</Text>
                                    </View>
                                    {this.falsaLabelComponent(item.falsa, item.hora)}
                                </View>
        ...
        if (this.state.groupsNumber == 0) {
            return (
                <View style={styles.container}>
                    <Text style={styles.noGroupText}>Usted no posee grupos creados, cree grupos tocando el ícono que esta arriba a la derecha</Text>
                </View>
            )
            ...
            return (
                <SafeAreaView style={styles.container}>
                    <View style={styles.alertContainer}>
                        <TouchableWithoutFeedback style={styles.alertBox} onPress={() => { this.alertPressHandler(this.state.userPhoneNumber, 1, this.state.groups) }}>
                        ...
                        <TouchableWithoutFeedback style={styles.alertBox} onPress={() => { this.alertPressHandler(this.state.userPhoneNumber, 2, this.state.groups) }}>
                        ...
                        <TouchableWithoutFeedback style={styles.alertBox} onPress={() => { this.alertPressHandler(this.state.userPhoneNumber, 3, this.state.groups) }}>
                    ...
                    <View style={styles.groupSectionContainer}>
                        ...
                        <FlatList data={this.state.groups}
                            keyExtractor={item => item.fk_grupo.toString()} renderItem={({ item }) => (
                                <TouchableWithoutFeedback onPress={() => { this.groupPressHandler(item.fk_grupo, item.isadmin, item.nombre) }}>
                                    <View style={styles.groupContainer}>
                                        <Image style={styles.groupIcon} width={screenWidth * 0.05} height={screenWidth * 0.05} source={require('../assets/iconosFigma/play-button.png')} />
